...
public final class GoalSpecification implements Specification<Goal> {

	private static final long serialVersionUID = 1L;

	public static Specification<Goal> getSpecification(Integer user, String description, BigDecimal valueIni, BigDecimal valueEnd,
		...
		return new Specification<Goal>() {
			...
			public Predicate toPredicate(Root<Goal> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(user != null) p00 = cb.and(cb.equal(root.<String>get("user"), user));

				if(description != null)
					p01 = cb.and(cb.like(root.<String>get("description"), "%" + CommonFunctions.toUnicode(description.toUpperCase()) + "%"));

				if(valueIni != null) {
					if(valueEnd != null) p02 = cb.and(cb.between(root.<BigDecimal>get("value"), valueIni, valueEnd));
					else p02 = cb.and(cb.equal(root.<BigDecimal>get("value"), valueIni));
				}
	...
	@Override
	public Predicate toPredicate(Root<Goal> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
		return null;
	}
